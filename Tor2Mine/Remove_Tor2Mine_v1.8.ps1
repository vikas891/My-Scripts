...
# >powershell.exe .\Remove_Tor2Mine_v1.8.ps1                                            #
#                                                                                       #
#Running the script with -ForceEnabled will not prompt and continue with removal        #
#>powershell.exe .\Remove_Tor2Mine_v1.8.ps1 -ForceEnabled                               #
#                                                                                       #
...
function Detect {
    ...
    Write-Host $ToRemove.Count "leftover(s) detected."
    "================================================"
    "Displaying Malicious Tasks Fragments in Registry"
    "================================================"
    Write-Host $ToRemove.Count "fragment(s) detected."
    $ToRemove
}
...
if ($schtasks.Count -gt 0 -or $servcount.Count -gt 0 -or $ProcessCount.Count -gt 0 -or $ToRemove.Count -gt 0) {
    ...
    if ($Confirm -eq 'y' -or $Force)     
    {
        ...
        if (Test-Path $Check)
            ...
            {
            "No Task Found pertaining to the registry remnant: $_ "
            }
        }
    }
  }
